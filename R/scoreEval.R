...
scoreEval_pert <- function(num_perts, data, set_U, Cov_e_M_e_D, K = c("linear", "poly", "gaussian") , ...){
  ...
  Ws_mat_M <- Ws_mat[order_time1,]
  Ws_mat_D <- Ws_mat_M[orderM_time2,]
...
scoreEval_pert_min_model <- function(num_perts, data, set_U, Cov_e_M_e_D, K = c("linear", "poly", "gaussian"), ...){
  ...
  data_new <- data[order(data[,ncol(data)-1]),]

  Ws_mat_new <- Ws_mat[order(data[,ncol(data)-1]),]

  M_new <- M_vec_pert(Ws_mat_new, Inf, data_new[,ncol(data_new)-1], data_new[, ncol(data_new)], c(0,0), as.matrix(data_new[,set_U]))

  K_rho <- kernelEval(t(data_new[,c(c(-1:-4), -set_U, -ncol(data_new) + 1, -ncol(data_new))]), K, ...)
