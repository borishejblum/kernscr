...
#'@param Ws optional inputed perturbations, should be a vector of length \code{N x num.perts} containing
...
#'
#'
#'@return a vector of p-values for 5 different models with names:\itemize{
...
#'compute_all_tests(data = mydata, num_perts=100, rho=rhos, kernel="gaussian")
#'# run the tests with linear kernel
#'compute_all_tests(data=mydata, num.perts=100, kernel="linear")
#'}
#'
compute_all_tests <- function(data, ind_gene=7:ncol(data), num_perts=1000, Ws=NULL, rho = NA,
  ...
  obtain_K_rhos <- function(K_rho){sqrt_ncol_K_rho <- ncol(K_rho); return( function(M_vec){return(matrix(t(M_vec)%*%t(K_rho), ncol = sqrt_ncol_K_rho, byrow = T)%*%(M_vec))})}
  obtain_K_rhos <- function(K_rho){
    sqrt_ncol_K_rho <- ncol(K_rho)
    return(function(M_vec){
      return(matrix(t(M_vec)%*%t(K_rho), ncol = sqrt_ncol_K_rho, byrow = T)%*%(M_vec))
    })
  }
  # the function with a fixed K_rho
  f_obtains_K_rhos <- obtain_K_rhos(K_rho)
  ## including cause specific hazard for M, D as well as marginal for D ##
  stats_all <- cbind("Mc"=f_obtains_K_rhos(t(M_Mc)), "Dc"=f_obtains_K_rhos(t(M_Dc)), "Dm"=f_obtains_K_rhos(t(M_Dm)))

  ## align perturbations with the ordered data
  Ws_mat <- matrix(Ws, dim(data)[1], num_perts); Ws_mat_M <- Ws_mat[order(data[,1]),]; Ws_mat_D <- Ws_mat_M[order(data_M[,2]),]
  ...
  stats.tune2_std <- sum(apply(stats_all_std[,c(1,3),drop=FALSE] ,2 ,max)) ## maximum statistic across rho and then outcome
  perts.tune2_std <- apply(matrix(unlist(lapply(stat_pert_std_list, function(xx){apply(xx, 1, max)})),ncol=3)[, c(1, 3)], 1, sum)
  pvals_tune2_std <- mean(perts.tune2_std > stats.tune2_std)
  ...
  stats.max_std <- apply(stats_all_std,2,max) ## maximum statistic across rho
  perts.max_std <- matrix(unlist(lapply(stat_pert_std_list,function(xx){apply(xx, 1, max)})), ncol=3) ## num_perts x 3 matrix
  pvals_max_std <- apply(perts.max_std > VTM(stats.max_std,num_perts),2,mean)
  ...
  f_obtains_K_rhos <- obtain_K_rhos(K_rho)
  stats_min <- f_obtains_K_rhos(t(M_min))
